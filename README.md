...
The list of rooms (filterable) is represented by RoomListModel, which contains summaries (one per room), and inside those the SDK room object itself. The RoomDetailsModel is what the RoomView itself assembles and uses to get precise details about the room (and the timeline itself).
